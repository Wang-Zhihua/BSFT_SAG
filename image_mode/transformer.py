...
class Bags(data_utils.Dataset):
    def __init__(self, patch_length=25000, clusters=10, usage='train', i_fold=-1):
        ...
        slide_file.close()
        print('load ' + self.usage + ' dataset:', len(self.label))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads=4, mlp_ratio=2, qkv_bias=False, drop_rate=0.1, drop_path=.0):
        super().__init__()
...
class Clinic_Branch(nn.Module):
    ...
    def __init__(self, clinic_dim=16, dim=64, mlp_ratio=2, drop_rate=0.3, cluster_num=10, num_classes=1):
        ...
        self.block = Block(dim=dim, mlp_ratio=mlp_ratio,  drop_rate=drop_rate)
        self.block1 = Block(dim=dim, mlp_ratio=mlp_ratio,  drop_rate=drop_rate)
...
class Muti_Modal(nn.Module):
    ...
    def forward(self, clinic):
        ...
        pred = self.clinic_head(clinic_feat[:, 0])
        return pred
...
def train(i_fold):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx,(label, clinic) in enumerate(train_loader,0):
        ...
        y_test_pred = []
        model.eval()
        for label, clinic in valid_loader:
            clinic = clinic.to(device)
            pred = model(clinic)
            l = int(label.item())
            y_test.append(l)
