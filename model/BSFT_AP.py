...
class Bottleneck_Fusion(nn.Module):
    ...
    def forward(self, image_feat, clinic_feat):
        ...
        return image_pred, clinic_pred, image_feat, clinic_feat
...
class Image_Branch(nn.Module):
    ...
    def __init__(self, patch_dim=512, dim=64, mlp_ratio=2, drop_rate=0.3, cluster_num=10, num_classes=1):
        ...
        self.act = nn.Sigmoid()
    def forward(self, x):
        rep = self.patch_to_embedding(x)
        feat = rep
        '''rep = rep.squeeze(0)
        A = self.attention(rep)
        A = torch.transpose(A, 1, 0)  # KxN
        x = torch.mm(A, rep)  # KxL
        feat = x.unsqueeze(0)
        b, n, _ = feat.shape
        feat += self.pos_embedding[:, :(n)]'''
        b, n, _ = feat.shape
...
class Muti_Modal(nn.Module):
    ...
    def __init__(self, cluster_num=10, num_classes=1, patch_dim=512, clinic_dim=16, dim=64, mlp_ratio=2, drop_rate=0.3):
        super().__init__()
        norm_layer=partial(nn.LayerNorm, eps=1e-6)
    ...
    def forward(self, x, clinic):
        clinic_pred_p, clinic_feat_p, clinic_feat = self.clinic_branch(clinic)
        image_pred_p, image_feat_p, image_feat = self.image_branch(x)
        image_pred_g, clinic_pred_g, image_feat_g, clinic_feat_g = self.fusion(image_feat, clinic_feat)
        ...
        return pred, image_pred_p, image_pred_g, image_feat_p, image_feat_g, clinic_pred_p, clinic_pred_g, clinic_feat_p, clinic_feat_g
