...
class Block(nn.Module):
    ...
    def forward(self, x):
        feat, attn = self.attn(self.norm1(x))
...
class Bottleneck_Fusion(nn.Module):
    ...
    def __init__(self, dim=64, num_heads=4, mlp_ratio=2, qkv_bias=True, drop_rate=0.3, num_classes=1, num_tokens=6):
        super().__init__()
        norm_layer=partial(nn.LayerNorm, eps=1e-6)
        self.fusion_token = nn.Parameter(torch.randn(1, num_tokens, dim))
    ...
    def forward(self, image_feat, clinic_feat):
        ...
        clinic_feat, clinic_attn = self.clinic_fusion(clinic_feat)
        #clinic_attn = (clinic_attn[0,0,0,1:18]+clinic_attn[0,1,0,1:18]+clinic_attn[0,2,0,1:18]+clinic_attn[0,3,0,1:18])*0.25
        fusion_token = clinic_feat[:, -self.num_tokens:]
        image_feat = torch.cat((image_token, image_feat, fusion_token), dim=1)
        image_feat, image_attn = self.image_fusion(image_feat)
        #image_attn = (image_attn[0,0,0,1:11]+image_attn[0,1,0,1:11]+image_attn[0,2,0,1:11]+image_attn[0,3,0,1:11])*0.25
        image_attn = (image_attn[0,0,1:11,1:11]+image_attn[0,1,1:11,1:11]+image_attn[0,2,1:11,1:11]+image_attn[0,3,1:11,1:11])*0.25
        '''
        fusion_token = image_feat[:, -self.num_tokens:]
        clinic_feat = clinic_feat[:, :-self.num_tokens]
        clinic_feat = torch.cat((clinic_feat, fusion_token), dim=1)
        clinic_feat, clinic_attn = self.clinic_fusion1(clinic_feat)
        
        fusion_token = clinic_feat[:, -self.num_tokens:]
        image_feat = image_feat[:, :-self.num_tokens]
        image_feat = torch.cat((image_feat, fusion_token), dim=1)
        image_feat, image_attn = self.image_fusion1(image_feat)
        
        fusion_token = image_feat[:, -self.num_tokens:]
        clinic_feat = clinic_feat[:, :-self.num_tokens]
        clinic_feat = torch.cat((clinic_feat, fusion_token), dim=1)
        clinic_feat, clinic_attn = self.clinic_fusion2(clinic_feat)'''
        
        image_feat = image_feat[:, 0]
        image_feat = self.image_norm(image_feat)
        image_pred = self.image_head(image_feat)
        image_pred = self.image_act(image_pred)
        clinic_feat = clinic_feat[:, 0]
        clinic_feat = self.clinic_norm(clinic_feat)
        clinic_pred = self.clinic_head(clinic_feat)
        clinic_pred = self.clinic_act(clinic_pred)

        return image_pred, clinic_pred, image_feat, clinic_feat, image_attn, clinic_attn
...
class Clinic_Branch(nn.Module):
    ...
    def __init__(self, clinic_dim=17, dim=64, mlp_ratio=2, drop_rate=0.3, cluster_num=10, num_classes=1):
        ...
        #self.block1 = Block(dim=dim, num_heads=4, mlp_ratio=mlp_ratio, qkv_bias=True, drop_rate=drop_rate) 
    ...
    def forward(self, clinic):
        ...
        x = self.dropout(x)
        x, attn = self.block(x)
        #attn = (attn[0,0,0,1:]+attn[0,1,0,1:]+attn[0,2,0,1:]+attn[0,3,0,1:])*0.25
        #x, attn = self.block1(x)
        feat_p = x[:, 0]
        feat_p = self.norm(feat_p)
        pred = self.head(feat_p)
        pred = self.act(pred)
        return pred, feat_p, feat, attn
...
class Image_Branch(nn.Module):
    ...
    def forward(self, x):
        ...
        x, attn = self.block(x)
        #attn = (attn[0,0,0,1:]+attn[0,1,0,1:]+attn[0,2,0,1:]+attn[0,3,0,1:])*0.25
        #attn = (attn[0,0,1:,1:]+attn[0,1,1:,1:]+attn[0,2,1:,1:]+attn[0,3,1:,1:])*0.25
        #x = self.block1(x)
        feat_p = x[:, 0]
        feat_p = self.norm(feat_p)
        pred = self.head(feat_p)
...
class Muti_Modal(nn.Module):
    ...
    def __init__(self, cluster_num=10, num_classes=1, patch_dim=512, clinic_dim=17, dim=64, mlp_ratio=2, drop_rate=0.3):
        super().__init__()
        norm_layer=partial(nn.LayerNorm, eps=1e-6)
        self.clinic_branch = Clinic_Branch(clinic_dim=clinic_dim, dim=dim, mlp_ratio=mlp_ratio, drop_rate=drop_rate, cluster_num=cluster_num, num_classes=num_classes)
        self.image_branch = Image_Branch(patch_dim=patch_dim, dim=dim, mlp_ratio=mlp_ratio, drop_rate=drop_rate, cluster_num=cluster_num, num_classes=num_classes)
        self.fusion = Bottleneck_Fusion(dim=dim, mlp_ratio=mlp_ratio, drop_rate=drop_rate, num_classes=num_classes)
    ...
    def forward(self, x, clinic):
        clinic_pred_p, clinic_feat_p, clinic_feat, clinic_attn_p = self.clinic_branch(clinic)
        image_pred_p, image_feat_p, image_feat, image_attn_p = self.image_branch(x)
        image_pred_g, clinic_pred_g, image_feat_g, clinic_feat_g, image_attn_g, clinic_attn_g = self.fusion(image_feat, clinic_feat)
        
        feat = torch.cat((image_feat_p, image_feat_g, clinic_feat_g, clinic_feat_p), dim=0)
        ...
        return pred, image_pred_p, image_pred_g, image_feat_p, image_feat_g, image_attn_p, image_attn_g, clinic_pred_p, clinic_pred_g, clinic_feat_p, clinic_feat_g, clinic_attn_p, clinic_attn_g

class Discriminator(nn.Module):

    def __init__(self, num_classes=1, dim=64):
        super().__init__()
        self.head = nn.Linear(dim, num_classes)
        self.act = nn.Sigmoid()
