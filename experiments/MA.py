...
class Muti_Modal(nn.Module):
    ...
    def __init__(self, cluster_num=10, num_classes=1, patch_dim=512, clinic_dim=16, dim=64, num_heads=4, mlp_ratio=2, qkv_bias=True, drop_rate=0.3):
        ...
        self.MA1 = Block(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop_rate=drop_rate)
        self.MA2 = Block(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop_rate=drop_rate)
    ...
    def forward(self, x, clinic):
        ...
        fusion_feat = torch.cat((x, c), dim=1)
        fusion_feat = self.MA1(fusion_feat)
        fusion_feat = self.MA2(fusion_feat)
        image_feat = self.image_norm(fusion_feat[:, 0])
        clinic_feat = self.clinic_norm(fusion_feat[:, 11])
